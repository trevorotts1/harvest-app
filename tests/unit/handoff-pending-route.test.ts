// T-R22 (remediation of the T-40R re-QC LOW/UX finding: POST /api/messaging/handoff/join was
// API-reachable ONLY, with no upline-facing UI to even see a pending bridge) — proves the NEW
// GET /api/messaging/handoff/pending read route (the read this remediation adds so `/team` has
// something real to render) is session-gated, reads no forged identity, and is strictly org/upline
// scoped by RE-DERIVING the caller's own organization from the database (never a client-supplied
// param) before handing it to the pre-existing, unmodified `ThreeWayHandoffService.visibleToUpline`
