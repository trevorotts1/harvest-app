// T-R22 (remediation of the T-40R re-QC LOW/UX finding) — the pre-existing POST
// /api/messaging/handoff/join route (built in T-40R) was, until this remediation, API-reachable
// ONLY: `tests/unit/messaging-surfaces-mount.test.ts` proves it exists, is session-gated, and wires
// `ThreeWayHandoffService.join`, but nothing ever actually INVOKED the handler to prove its
// contract end to end. This suite closes that gap for the exact affordance the new `/team` page
// (this remediation) now calls: proves the OWNERSHIP 404 (a caller who is not this handoff's
// invited upline gets the SAME "Handoff not found" 404 as a truly nonexistent id — no leak of which
// case it was), the 409 NOT_JOINABLE case, and the happy JOIN path — using the SESSION user id as
// the joiner, never a forged header. This file does not modify the route or the service — it only
// adds test coverage.
