...
`VERCEL_ORG_ID`, `VERCEL_PROJECT_ID` (T-02/T-60). The `deploy` job carries an `if:` guard that
evaluates false while any of those three secrets is unset, so GitHub Actions marks the job
**skipped**, not failed — this file never breaks the pipeline in the meantime. Once all three
secrets exist, the guard flips on automatically with no file edit required, and production
promotion runs from `main` per master spec §2.2 (Vercel hosting; preview deploys on PRs are a
follow-on once the Vercel project is linked).
